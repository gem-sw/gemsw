...
// constructor
//
MuonSmoother::MuonSmoother(const ParameterSet& par, const MuonServiceProxy* service) : theService(service) {
  ...
  category_ = "Muon|RecoMuon|CosmicMuon|MuonSmoother";
}

//
// destructor
//
MuonSmoother::~MuonSmoother() {
  if (theUpdator)
    delete theUpdator;
  if (theUtilities)
    delete theUtilities;
...
//
Trajectory MuonSmoother::trajectory(const Trajectory& t) const {
...
// fit and smooth trajectory
//
std::vector<Trajectory> MuonSmoother::trajectories(const TrajectorySeed& seed,
...
// fit trajectory
//
vector<Trajectory> MuonSmoother::fit(const Trajectory& t) const {
  if (t.empty())
    return vector<Trajectory>();
...
// fit trajectory
//
vector<Trajectory> MuonSmoother::fit(const TrajectorySeed& seed,
                                           const ConstRecHitContainer& hits,
                                           const TrajectoryStateOnSurface& firstPredTsos) const {
...
// smooth trajectory
//
vector<Trajectory> MuonSmoother::smooth(const vector<Trajectory>& tc) const {
...
// smooth trajectory
//
vector<Trajectory> MuonSmoother::smooth(const Trajectory& t) const {
  if (t.empty()) {
...
TrajectoryStateOnSurface MuonSmoother::initialState(const Trajectory& t) const {
  if (t.empty())
...
void MuonSmoother::sortHitsAlongMom(ConstRecHitContainer& hits, const TrajectoryStateOnSurface& tsos) const {
  if (hits.size() < 2)
    return;
