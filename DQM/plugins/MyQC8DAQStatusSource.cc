#include "gemsw/DQM/plugins/MyQC8DAQStatusSource.h"

using namespace std;
using namespace edm;

MyQC8DAQStatusSource::MyQC8DAQStatusSource(const edm::ParameterSet &cfg)
...
void MyQC8DAQStatusSource::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
  ...
  descriptions.add("MyQC8DAQStatusSource", desc);
}

void MyQC8DAQStatusSource::SetLabelAMCStatus(MonitorElement *h2Status) {
  if (h2Status == nullptr) {
    return;
  }
...
void MyQC8DAQStatusSource::SetLabelOHStatus(MonitorElement *h2Status) {
  if (h2Status == nullptr) {
    return;
  }
...
void MyQC8DAQStatusSource::SetLabelVFATStatus(MonitorElement *h2Status) {
  if (h2Status == nullptr) {
    return;
  }
...
void MyQC8DAQStatusSource::bookHistograms(DQMStore::IBooker &ibooker, edm::Run const &, edm::EventSetup const &iSetup) {
...
void MyQC8DAQStatusSource::fillSummary(int xBin,
...
void MyQC8DAQStatusSource::analyze(edm::Event const &event, edm::EventSetup const &eventSetup) {
...
DEFINE_FWK_MODULE(MyQC8DAQStatusSource);
